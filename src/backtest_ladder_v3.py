"""Low-frequency batch position sizing strategy V3 for 159632."""
...
def run_ladder_v3_backtest(
    ...
    for i in range(len(df)):
        ...
        target_position = min(base_target, max_position)

        df.loc[i, "target_position"] = target_position
        ...
        if abs(delta) >= rebalance_threshold and cooldown_counter == 0:
            ...
            if liquidity_ok:
                ...
                if abs(delta) >= min_trade_delta or target_position == 0:
                    if delta_value > 0:
...
def save_reports(symbol: str, daily_df: pd.DataFrame, trades_df: pd.DataFrame) -> None:
    ...
    equity_path = REPORT_DIR / f"ladder_v3_{symbol}_equity.csv"
    trades_path = REPORT_DIR / f"ladder_v3_{symbol}_trades.csv"
    chart_path = REPORT_DIR / f"ladder_v3_{symbol}_equity.png"

    daily_df.to_csv(equity_path, index=False, encoding="utf-8-sig")
    trades_df.to_csv(trades_path, index=False, encoding="utf-8-sig")

    plt.figure(figsize=(12, 6))
    plt.plot(daily_df["date"], daily_df["strategy_equity"], label="Ladder V3")
    plt.plot(daily_df["date"], daily_df["benchmark_equity"], label="Buy & Hold")
    plt.title(f"{symbol} Ladder V3 Strategy")
    plt.xlabel("Date")
    plt.ylabel("Equity")
    plt.legend()
    plt.tight_layout()
